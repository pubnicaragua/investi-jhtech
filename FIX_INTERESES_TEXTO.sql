...
RETURNS TABLE (
  ...
  role TEXT,
  nivel_finanzas TEXT,
  intereses TEXT[],  -- CAMBIO: TEXT[] en lugar de UUID[]
  score_total INT,
  score_nivel INT,
  score_intereses INT,
  score_metas INT,
...
BEGIN
  ...
  WITH candidate_users AS (
    SELECT DISTINCT
      ...
      u.role,
      u.nivel_finanzas,
      -- CAMBIO: Devolver TEXT[] directamente, NO convertir a UUID[]
  ...
  SELECT 
    ...
    cu.intereses,  -- TEXT[] directamente
...
-- PASO 3: Verificar
SELECT 'Función creada con TEXT[] para intereses' as status;
